...
namespace UnitTest.IRCParser
{
	...
	class ParserTest
	{
		...
		public void QueryParseTestWithOneTargetAndTrailngParams()
		{
			...
			bool parsed = IRCSharp.Kernel.Parser.IRC.IRCQueryParser.TryParse("network",line, out query);

			Assert.AreEqual("network", query.Network);
			...
			Assert.AreEqual("#mslot.dk", query.Channel);
			Assert.True(parsed);
		...
		public void QueryParseTestWithNoPreifxAndNoTrailing()
		{
			...
			Assert.AreEqual(String.Empty, query.Channel);
			Assert.True(parsed);
		...
		public void QueryParseTestWithJoin()
		{
			...
			Assert.AreEqual("#Twilight_zone", query.Channel);
			Assert.True(parsed);
		...
		public void QueryParseTestWithJoinClientSide()
		{
			...
			Assert.AreEqual("#foo,#bar", query.Channel);
			Assert.True(parsed);
		...
		[Test]
		public void TestQueryWhereChannelIsOnLetter()
		{
			//:mslot2!~mslot2@56344eba.rev.stofanet.dk PRIVMSG #c :!command arg1 arg2 arg3
			string line = ":mslot2!~mslot2@56344eba.rev.stofanet.dk PRIVMSG #c :!command arg1 arg2 arg3";
			IRCSharp.Kernel.Model.Query.IRCCommandQuery query = null;
			bool parsed = IRCSharp.Kernel.Parser.IRC.IRCQueryParser.TryParse("network", line, out query);

			Assert.AreEqual("network", query.Network);
			Assert.AreEqual("mslot2!~mslot2@56344eba.rev.stofanet.dk", query.Prefix);
			Assert.AreEqual(IRCSharp.Kernel.Model.Query.IRCCommand.PRIVMSG, query.Command);
			Assert.AreEqual("#c :!command arg1 arg2 arg3", query.Parameter);
			Assert.AreEqual("#c", query.Channel);
			Assert.True(parsed);
		}
